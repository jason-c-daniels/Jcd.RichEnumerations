...
public class FlagEnumTests
{
   ...
   [InlineData(2, "Flag2")]
   [InlineData(4, "Flag3")]
   public void ToString_Returns_Expected_String(ulong value, string expected)
   {
      Assert.Equal(expected, ((MyFlagEnum) value).ToString());
   }

   [Fact]
   public void GetHashCode_Returns_Expected_Value()
   {
      Assert.Equal(MyFlagEnum.Flag4.Value.GetHashCode() ^ MyFlagEnum.Flag4.GetType().GetHashCode(), MyFlagEnum.Flag4.GetHashCode());
   }
