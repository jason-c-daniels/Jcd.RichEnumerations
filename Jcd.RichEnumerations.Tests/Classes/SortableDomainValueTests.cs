...
public class SortableDomainValueTests
{
   ...
   {
      Assert.Equal(expected, ((MySortableValue) value).ToString());
   }

   [Fact]
   public void GetHashCode_Returns_Expected_Value()
   {
      Assert.Equal(MySortableValue.ValueTen.Value ^ MySortableValue.ValueTen.GetType().GetHashCode(), MySortableValue.ValueTen.GetHashCode());
   }
