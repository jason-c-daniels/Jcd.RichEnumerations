...
public sealed class CreditCard
   (int id, string? name, CreditCardBrand brand, string? bankName, string? cardNumber, string? ccv, string? expirationDate, bool isNew = false)
   : RichEnumBase<CreditCard, CreditCard>
   , IEquatable<CreditCard>
{
   public static readonly CreditCard MyAmex1 = new(1, "Capitaleers Amex", CreditCardBrand.AmericanExpress, "Capitaleers", "1234-5678-9012-5678", "123", "2029-01");

   public static readonly CreditCard MyAmex2 = new(2, "Finmong Amex", CreditCardBrand.AmericanExpress, "Finacemongers", "2345-6789-0125-6781", "234", "2027-07");

   public static readonly CreditCard BlathersVisa = new(3, "My Visa", CreditCardBrand.Visa, "Blather's Group LLC", "3456-7890-1256-7812", "345", "2026-03");

   #region IEquatable<CreditCard> Members
   ...
   {
      return $"ID: {Id}; Name: {Name}; Brand: {Brand.Name}; Exp: {ExpirationDate}";
   }
   ...
   {
      return All.FirstOrDefault(b => string.Compare(b.Name,       key, StringComparison.InvariantCultureIgnoreCase) == 0
                                  || string.Compare(b.CardNumber, key, StringComparison.InvariantCultureIgnoreCase) == 0
                                  || (int.TryParse(key, out var parsedId) && b.Id == parsedId)
                               );
   }
   ...
   public static bool AddCard(CreditCard newCard)
   {
      ...
      card ??= All.FirstOrDefault(c => string.Compare(c.Name, newCard.Name, StringComparison.InvariantCultureIgnoreCase) == 0);
   ...
   {
      return new CreditCard(Id, Name, Brand, BankName, CardNumber, Ccv, ExpirationDate, IsNew);
   }
   ...
   public CreditCard()
      : this("UNSET", CreditCardBrand.Unset, "UNSET", "UNSET", "UNSET", "UNSET")
   {
   }

   public CreditCard(string? name, CreditCardBrand brand, string? bankName, string? cardNumber, string? ccv, string? expirationDate)
      : this(All.Max(i => i.Id) + 1, name, brand, bankName, cardNumber, ccv, expirationDate)
   {
   }
