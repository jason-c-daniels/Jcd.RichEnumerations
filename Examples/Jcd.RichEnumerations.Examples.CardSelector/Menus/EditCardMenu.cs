...
public class EditCardMenu : Menu<EditCardMenu, EditCardMenu.Item, EditCardMenu.Result>
{
      ...
      , IMenuProcessingResult<Result>
   {
      ...
      public static readonly Result UnmappedCommand = new(int.MinValue
                                                        , "A command hook was not mapped."
                                                        , "An attempt to change some data failed."
                                                        , true
                                                         );
   ...
   protected override (Item? command, string args, string cmdText) ParseCommand(string? input)
   {
      ...
      return item != null ? (item, results[1], results[0]) : ParseByName(input);
   }
   ...
   private static (Item? command, string args, string cmdText) ParseByName(string input)
   {
      ...
      foreach (var item in Item.All)
      {
         ...
         if (input.StartsWith(item.Name, StringComparison.InvariantCultureIgnoreCase))
            return (item, input.Remove(0, item.Name.Length).Trim(), input[..item.Name.Length]);
      }
